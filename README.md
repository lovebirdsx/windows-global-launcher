...
* 针对高负载场景（大型游戏、编译、索引等）做了响应优化：Alt+Tab 由专用短路径钩子接管、窗口列表先显示后补图标、进程优先级提升为 AboveNormal，按键延迟显著降低
...
* 双击或 `Esc` 关闭（**双击关闭前自动把内容写回剪贴板**，误关后 `F7`/粘贴即可找回，剪贴板历史也会置顶收录；`Esc`、右键菜单、`Del` 删除、关闭所有为明确意图、不覆盖剪贴板）；右键菜单：复制图像 / 保存为文件 / 缩放 100% / 关闭所有贴图
* 文字贴图：窗口宽高随内容自适应（宽最多 480，超出折行；高最多屏幕工作区的 60%，超出滚动）；滚轮滚动内容（`Ctrl+滚轮` 仍调透明度）、不支持缩放；右键菜单「编辑」修改文字（`Enter` 保存、`Shift+Enter` 换行、`Esc` 取消、点击别处自动保存），**进入编辑时窗口自动放大到最大尺寸方便编辑**（左上角固定不动，靠近屏幕右/下边缘时以到边缘的可用空间为准；编辑中保持最大、内容超高滚动，落定后按内容缩回原尺寸，位置全程不变），另有复制文本 / 保存为文件（txt）/ 分类子菜单（8 种颜色任选，仅描边颜色区分，默认灰）/ 关闭所有贴图
* `Shift+F7` 隐藏/显示所有贴图（图片贴图与文字便签统一切换；整体隐藏时新钉一张贴图会连同旧贴图一起恢复显示）；托盘菜单「隐藏所有贴图/显示所有贴图」同效（无贴图时置灰）
* `Win+Q`（或托盘菜单「框选移动贴图」）进入框选态：全屏拖蓝色虚线橡皮筋框，松手选中与框相交的贴图（描边加粗、颜色继承归属色：文本便签为分类色、图片贴图为白色），之后拖动任一选中贴图即整体移动；点击空白处或按 `Esc` 取消选中（无论焦点在哪；再按一次 `Esc` 关闭当前贴图）；点击其中一张贴图改为只选中它；按 `Del` 删除所有选中的贴图/便签（无论焦点在哪；仅裸 `Del`，`Shift+Del` 等带修饰键组合不触发；编辑中的便签不受影响）；隐藏中的贴图不参与框选；框选选中多个文字便签后，右键任一选中便签的「复制文本」会复制所有选中便签的文本（按屏幕位置从上到下、从左到右，换行分隔）
