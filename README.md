...
* 文字贴图：窗口宽高随内容自适应（宽最多 480，超出折行；高最多屏幕工作区的 60%，超出滚动）；滚轮滚动内容（`Ctrl+滚轮` 仍调透明度）、不支持缩放；右键菜单「编辑」修改文字（`Enter` 保存、`Shift+Enter` 换行、`Esc` 取消、点击别处自动保存），编辑时窗口随输入实时伸缩，另有复制文本 / 保存为文件（txt）/ 关闭所有贴图
